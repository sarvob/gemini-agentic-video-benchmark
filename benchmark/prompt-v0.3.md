You are producing a candidate edit plan for a long-form creator video. This is protocol v0.3.
...
Omit `keep_start_sec` and `keep_end_sec` only for a `cut` decision. Return every supplied question ID and candidate segment ID exactly once. Every factual answer, moment, and event must cite a time range. Every kept range must remain within its candidate segment. `constraints.target_duration_sec` must equal the exact sum of all returned kept ranges. Do not round or estimate this sum.
